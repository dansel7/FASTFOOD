...
// The visitor has clicked the PayPal checkout button
	...
	*/
	...
	/*
		Metodos de insersion de la base de datos
	*/
	
	foreach ($jcart->get_contents() as $item) {
		$_POST['id_item']	    = $item['id'];
		$_POST['item_precio']	= $item['price'];
		$_POST['cantidad_item']	= $item['qty'];
		$_POST['usuario_id']	= $item['qty'];
		$_POST['creacion']	    = date("Y-m-d H:i:s");
		
		$query_new_item = $conexion->formulario_a_database($enlace, "item_venta", 'jcartToken, jcartItemId, jcartItemQty, jcartItemName, jcartItemPrice, jcartIsCheckout, jcartPaypalCheckout, ', '', 'insert');
	}
	...
	elseif ($validPrices === true) {
		...
		$queryString .= "&business=" . urlencode('hugodimas11-facilitator@hotmail.com');
